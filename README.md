...
| 명령 | 설명 |
|---|---|
| `llmwiki reg schema` | 규제 온톨로지(노드 13종·관계 15종·유보 트리거) 보기 |
...
뷰어에서는 좌측 하단 **규제 준수 평가 열기 →** (`/reg/services`) 로 들어갑니다.

| 탭 | 보여 주는 것 |
|---|---|
| 서비스 | 서비스 목록·정의(프로그램 묶기)·서비스별 진행 단계와 다음 할 일 |
| 위험등급 | STEP 1~5 마법사 + 코드 근거 제안 |
...
### 서비스 — 소스 분석과 규제 검증이 만나는 자리

규제는 프로그램이 아니라 **AI 서비스** 단위로 묻습니다. 4축 프로파일도 위험등급도
서비스 하나에 붙는 값이라, 이 경계가 흔들리면 등급이 흔들립니다. 그래서 묶는 행위는
사람의 것입니다 — `Service` 와 `REALIZED_BY` 는 sLM 이 제안할 수 없고(`llm_proposable=False`),
검증기가 그것을 막습니다.

    ① 소스 분석 ─┐
                 ├→ ② 서비스 정의 → ③ 위험등급 → ④ 통제·증적 → ⑤ 판정 → ⑥ 결재·확정
       규제 원문 ─┘

진행 단계는 화면 전체가 아니라 **서비스마다 따로** 돕니다. 서비스 A 가 ③ 에 있고
서비스 B 가 ⑥ 에 있을 수 있습니다(`/svc/<uuid>` 대시보드).

| 엔드포인트 | 하는 일 |
|---|---|
| `GET /api/reg/programs` | 서비스로 묶을 후보 — 정적 분석이 뽑은 운영 프로그램(이미 묶인 서비스 포함) |
| `GET /api/reg/services` | 서비스 목록 + 서비스별 단계 |
| `GET /api/reg/service/{uuid}` | 대시보드 — 등급·통제·수기 의존 증적·판정·대기 결재·다음 할 일 |
| `POST /api/reg/services/propose` | 프로그램 묶음 → 서비스 정의 **제안** (ChangeSet 으로 결재 큐에 올라갑니다) |
| `POST /api/reg/risk/suggest` | 코드 근거 제안 — 프로파일 2축과 데이터·위탁 관련 위험 항목 **후보** |

`link-programs` 는 CLI 에만 있어 사용자가 그 기능의 존재를 알 수 없었습니다. 이제
서비스 정의 화면이 같은 일을 하되, 어느 프로그램을 한 서비스로 볼지는 사람이 고릅니다.

### 코드 근거는 제안이지 판정이 아닙니다

코드가 `TB_CUST` 를 읽는다는 사실은 "개인정보를 처리한다" 는 **신호**지 "개인정보를
부적절하게 처리한다" 는 **판정**이 아닙니다. 그래서 화면은 체크박스를 미리 켜지 않고
근거 링크와 함께 제안만 하며(점선 테두리), 사람이 누르는 순간 실선이 됩니다.

| 코드가 아는 것 | 쓰이는 자리 | 신뢰도 |
|---|---|---|
| 계층 + URL 패턴 | 프로파일 1-1 사용자 범위 | 후보 |
| 접근 테이블 + CRUD | 프로파일 1-4 데이터 민감도 | 후보 |
| 개인정보 테이블 읽기·쓰기 | 위험 항목 4·5·30·32 | 후보 |
| 외부 호출 지점 | 위험 항목 28·29 | 후보 |
| 성능·드리프트 측정 코드 유무 | 위험 항목 14·16 | 후보 |
| — | STEP 1 고영향 · 프로파일 1-2 · 1-3 | **불가** (화면에 명시) |

패턴은 `llmwiki/compliance/data/code_hints.yaml` 이 단일 원본입니다 — 명명규칙은
현장마다 다르므로 코드가 아니라 데이터로 두었습니다.
